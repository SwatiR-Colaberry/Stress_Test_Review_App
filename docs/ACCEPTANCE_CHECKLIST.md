...
## Audit and History
- [ ] REQ-008 — a completed review's AI draft and human edits are both preserved and retrievable by an audit request — a test archives a completed review and asserts both are present (STORY-007 AC1/AC2); not built. Note (2026-09-25): STORY-011's audit-trail criterion now passes — `backend/app/audit/trail.py` records every intake outcome and every finalize-check attempt as an append-only `AuditEvent` in git-ignored `data/audit/audit_trail.jsonl` (not SQL Server: existing tables and procedures must not change). That is an *action* log; REQ-008's preserved AI drafts and human edits (STORY-007) are still not built.

## Data Retrieval
- [x] REQ-004 — a Basecamp submission fetch includes comments, attachments, and links; an empty project returns an empty dataset — `backend/app/basecamp/test_submission_retrieval.py` asserts both (STORY-002 AC1/AC2), plus the audit log with timestamp and user id (AC3); since 2026-09-25 each retrieval is also recorded in the append-only audit trail (`retrieval_started` / `_completed` / `_failed`, STORY-011). **Verified against simulated Basecamp API responses only (session CC-20260925-dpme, 2026-09-25): not yet run on live data, because no Basecamp OAuth app is registered.** Live check: `backend/scripts/retrieve_submissions.py --project-id <id> --user-id <you>`
...
## Marker Detection
- [x] REQ-001 — `##Critique##` is detected in a Basecamp comment with reasonable spacing/case variants normalized — `backend/app/basecamp/test_critique_marker_detector.py` (9/9 passing) asserts the four documented spacing variants and case variants trigger, and `Critique`/`#Critique`/`##Review##`/ordinary prose don't; also exercised over HTTP via `POST /basecamp/critique-marker/detect` (STORY-001 AC1/AC2, detection half). Since 2026-09-25 (user decision): `##Please Critique##` (any case/spacing) also creates the review item, flagged non-standard with a `marker_note` telling the reviewer to remind the student to write `##Critique##` next time, and audited with reason `NONSTANDARD_MARKER` (`classify_critique_marker`; the 5 real such comments in the history are now all caught, previously 4 were missed). `#Critique#` and plain prose still do not count.
...
## Rule Application
- [x] REQ-003 — loading rules for an ST0 submission returns only ST0-prefixed rules; an unknown Stress Test routes to manual resolution — a test asserts both (STORY-003 AC1/AC2) — `backend/app/rules/test_loader.py` and `test_module.py` (2026-09-25): `load_rules()` loads only `ST0/v1.json` (built word for word from `docs/stress-test-rules/ST0-rules-v1.md`), routes unknown/unclear Stress Tests and every load failure to manual resolution with a reason code, and records each load with its rule version in the audit trail. Not yet called by a review flow (STORY-004). See `directives/rule-modules.md`.

## Security
- [x] REQ-016 — no credentials are stored in source code or shared documents — `python3 backend/scripts/scan_for_credentials.py` exits 0 ("No credential-shaped content found across 100 text files", 2026-09-25); `backend/app/guardrails/test_credential_leak_guardrail.py` (12/12) passing; also exercised over HTTP via `POST /security/scan-credentials`; audit events have no free-text field and `data/audit/` is git-ignored; `.colaberry/progress.json`'s STORY-011 entry has this criterion locally marked `"passed": true`
