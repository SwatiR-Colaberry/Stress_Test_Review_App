...
**Stress Test Review Application** — A controlled review platform integrating Basecamp and human reviewers, with AI-assisted draft reviews.
...
- **REQ-017** — The system must support adding new Stress Test modules (ST1–ST5) as configuration without rebuilding the core application.
...
- **REQ-008** — The system must preserve the history of each review, including AI drafts and human edits, for audit purposes.
- **REQ-010** — The system must retry external API calls safely within a bounded number of attempts and surface visible errors if they fail.
- **REQ-011** — The system must not auto-approve any submission; human review is mandatory.
- **REQ-016** — The system must ensure that no credentials are stored in source code or shared documents.
...
- **REQ-001** (FUNC, must) — The system must detect a '##Critique##' marker in Basecamp comments, normalizing reasonable spacing and case variants.
- **REQ-002** (FUNC, must) — The system must create a Review Queue item with status 'Pending' for each detected critique marker, tied to the exact submission/version.
- **REQ-003** (FUNC, must) — The system must load only the rule module for the identified Stress Test (ST0) and never apply generic or incorrect rules.
- **REQ-004** (FUNC, must) — The system must retrieve all necessary submission data from Basecamp, including comments, attachments, and links.
- **REQ-005** (FUNC, must) — Claude must evaluate submissions against loaded rules, returning structured draft findings with rule ID, status, severity, evidence, reason, and confidence score.
- **REQ-006** (FUNC, must) — Human reviewers must be able to approve, edit, reject, or add findings before final feedback is posted back to Basecamp.
- **REQ-007** (FUNC, must) — The system must mark reviews as 'Completed' only after human review and Basecamp posting are both done.
- **REQ-008** (SAFE, must) — The system must preserve the history of each review, including AI drafts and human edits, for audit purposes.
- **REQ-009** (FUNC, must) — The system must route ambiguous project or Stress Test identifications to manual resolution rather than guessing.
- **REQ-010** (SAFE, must) — The system must retry external API calls safely within a bounded number of attempts and surface visible errors if they fail.
- **REQ-011** (SAFE, must) — The system must not auto-approve any submission; human review is mandatory.
- **REQ-012** (CONSTRAINT, must) — The system must use Basecamp's OAuth 2.0 for authentication and authorization.
- **REQ-013** (CONSTRAINT, must) — The system must connect to Microsoft SQL Server for data storage and retrieval.
- **REQ-014** (CONSTRAINT, must) — The system must use Claude via the Anthropic API for AI evaluations.
- **REQ-015** (CONSTRAINT, must) — The system must expose a typed API using FastAPI and Pydantic models.
