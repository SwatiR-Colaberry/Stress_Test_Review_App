...
const ENTITIES = [
  ...
  },
  {
    ...
    note: "Built (STORY-011; rule-loading events from STORY-003): one append-only record per action taken on a submission. Stored as a git-ignored JSON Lines file (data/audit/audit_trail.jsonl), not a SQL Server table — existing tables and procedures must not change. Ids and outcomes only; no free text, so no comment bodies or secrets.",
    fields: [
      ["event_id", "uuid", "primary key"],
      ["recorded_at", "timestamp", ""],
      ["action", "enum", "review_created | review_already_queued | comment_no_marker | comment_rejected_malformed | finalize_allowed | finalize_blocked | retrieval_started | retrieval_completed | retrieval_failed | rules_loaded | rules_manual_resolution"],
      ...
      ["project_id", "bigint", "nullable — Basecamp project (retrieval events)"],
      ["stress_test_id", "string", "nullable — e.g. ST0 (rule-loading events)"],
      ["rule_version", "string", "nullable — e.g. v1 (rule-loading events)"],
